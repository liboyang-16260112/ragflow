  ...
  translation: {
    ...
    knowledgeDetails: {
      ...
      similarityThresholdTip:
        'FMoss-RAG 在檢索時會使用加權關鍵詞相似度與加權向量餘弦相似度的組合；選擇重排序模型時，則使用加權關鍵詞相似度與加權重排序分數的組合。此參數設定使用者查詢與文字區塊之間的相似度閾值。相似度分數低於此閾值的文字區塊將從結果中排除。預設閾值為 20，這表示只有混合相似度分數達到 20 或以上的文字區塊才會被檢索。如果向量相似度權重設定為 0，則此閾值不適用。',
...
以上就是你需要總結的內容。`,
 ...
 NER：使用 spaCy NER 和基於規則的關鍵詞提取來抽取實體和關係，無需 LLM 參與提取過程，速度快且資源消耗低`,
    ...
    setting: {
      ...
      embeddingModel: '嵌入模型',
      embeddingModelTip:
        '如未顯示可選模型，請檢查你是否在使用 FMoss-RAG slim 版（不含嵌入模型）；或根據 https://ragflow.io/docs/dev/supported_models 確認你的模型供應商是否提供該模型。',
      ...
      sitemapUserAgentTip:
        '每次請求送出的 User-Agent 標頭。留空則使用 FMoss-RAG-SitemapConnector/1.0。',
      sitemapBatchSizeTip: '每批擷取並送到 FMoss-RAG 的頁面數量。',
