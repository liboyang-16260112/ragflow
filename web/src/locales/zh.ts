export default {
  translation: {
    ...
    login: {
      ...
      register: '创建账户',
      continue: '继续',
      heroTitle: 'FMoss 智能知识增强引擎',
      heroSubtitle: '打通非结构化数据与 LLM 之间的精准检索链路',
      start: '立即开始',
      description:
...
例如：一条带有 1024 维 Embedding 的 1 KB 消息大约使用 9 KB。5 MB 的默认限制大约可容纳 500 条此类消息。`,
    ...
    knowledgeDetails: {
      ...
      retrievalTestingDescription:
        '进行检索测试，检查 FMoss-RAG 是否能够为大语言模型（LLM）恢复预期的内容。',
      Parse: '解析',
      dataset: '知识库',
      testing: '检索测试',
      configuration: '配置',
      ...
      similarityThresholdTip:
        'FMoss-RAG 在检索时会使用加权关键词相似度与加权向量余弦相似度的组合；选择重排序模型时，则使用加权关键词相似度与加权重排序分数的组合。此参数用于设置用户查询与文本块之间的相似度阈值。相似度分数低于此阈值的文本块将从结果中排除。默认阈值为 20，也就是说，只有混合相似度分数达到 20 或以上的文本块才会被检索。如果向量相似度权重设置为 0，则此阈值不适用。',
      vectorSimilarityWeight: '向量相似度权重',
...
<p>编译后的知识单元以分块形式合并进分块流输出，适合在分块文档之上构建可检索的知识层。</p>`,
    ...
    <p>
      ...
      您只需与<i>'FMoss-RAG'</i>交谈即可列出所有符合资格的候选人。
      </p>
        `,
...
示例：general/v2/`,
      ...
      sitemapUserAgentTip:
        '每次请求发送的 User-Agent 请求头。留空则使用 FMoss-RAG-SitemapConnector/1.0。',
      sitemapBatchSizeTip: '每批抓取并发送到 FMoss-RAG 的页面数量。',
      ...
      embeddingModelTip:
        '所有新创建的知识库使用的默认 Embedding 模型。如未显示可选模型，请检查你是否在使用 FMoss-RAG slim 版(不含 Embedding 模型)；或根据 https://ragflow.io/docs/dev/supported_models 确认你的模型供应商是否提供该模型。',
      img2txtModel: 'VLM',
      img2txtModelTip:
