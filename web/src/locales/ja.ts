  ...
  translation: {
    ...
    knowledgeDetails: {
      ...
      similarityThresholdTip:
        'FMoss-RAGは検索時に、重み付けされたキーワード類似度と重み付けされたベクトルコサイン類似度の組み合わせ、またはリランカーモデルが選択されている場合は、重み付けされたキーワード類似度と重み付けされたリランキングスコアの組み合わせを使用します。このパラメーターは、ユーザークエリとチャンク間の類似度のしきい値を設定します。類似度スコアがこのしきい値を下回るチャンクは結果から除外されます。デフォルトでは、しきい値は20に設定されています。つまり、ハイブリッド類似度スコアが20以上のチャンクのみが検索されます。ベクトル類似度の重みが0に設定されている場合、このしきい値は適用されません。',
      ...
      rerankTip: `任意です。空欄の場合、FMoss-RAGは加重キーワード類似度と加重ベクトルコサイン類似度の組み合わせを使用します。リランキングモデルが選択された場合は、加重リランキングスコアが加重ベクトルコサイン類似度に代わります。リランキングモデルを使用すると、システムの応答時間が大幅に増加することにご注意ください。`,
      topK: 'トップK',
      ...
      retrievalTestingDescription:
        '検索テストを実行し、FMoss-RAGがLLMに必要な内容を正しく取得できるかを確認します。',
      Parse: 'パース',
...
上記が要約する内容です。`,
      ...
      tableColumnModeAutoDescription:
        'すべての列がチャンクテキストに含まれ、メタデータとしても保存されます（FMoss-RAGのデフォルト）。',
    ...
    chat: {
      ...
      variable: '変数',
      variableTip: `FMoss-RAGのチャットアシスタント管理APIと組み合わせて使用することで、変数はより柔軟なシステムプロンプト戦略を開発するのに役立ちます。定義された変数は、LLMのプロンプトの一部として「システムプロンプト」で使用されます。{knowledge}は、指定された知識ベースから取得された部分を表す特別な予約変数であり、「システムプロンプト」ではすべての変数を波括弧{}で囲む必要があります。詳細はhttps://ragflow.io/docs/chat_configuration#system-promptを参照してください。`,
      add: '追加',
      key: 'キー',
      optional: 'オプション',
      operation: '操作',
      ...
      betaError: 'システム設定ページからFMoss-RAG APIキーを取得してください。',
      searching: '検索中...',
      parsing: '解析中',
      uploading: 'アップロード中',
    ...
    setting: {
      ...
      embeddingModelTip:
        '新しく作成された各ナレッジベースのデフォルト埋め込みモデルです。ドロップダウンから埋め込みモデルが見つからない場合は、FMoss-RAGスリムエディション（埋め込みモデルを含まない）を使用しているか、https://ragflow.io/docs/dev/supported_models を確認して、モデルプロバイダーがこのモデルをサポートしているかどうかを確認してください。',
    ...
    fileManager: {
      ...
      uploadDescription:
        'FMoss-RAGは、単一またはバッチでのファイルアップロードをサポートします。ローカルにデプロイされた FMoss-RAG の場合: アップロードごとの合計ファイルサイズ制限は 1GB、バッチアップロードの制限は 32 ファイルです。アカウントごとのファイル総数には制限がありません。cloud.ragflow.io の場合: アップロードごとの合計ファイルサイズ制限は 10MB、各ファイルは 10MB を超えず、アカウントごとに最大 128 ファイルまでです。',
